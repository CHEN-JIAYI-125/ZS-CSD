...
# ---------------------------------------------------------------------------
# v5 — implicit latent relation matrix + augmented GCN on topology graph
# ---------------------------------------------------------------------------

def build_explicit_adjacency(graph, num_nodes, device, dtype):
    """Row-normalized adjacency from all explicit topology edges."""
    adj = torch.zeros(num_nodes, num_nodes, device=device, dtype=dtype)
    edge_index = _graph_tensor(graph, 'edge_index')
    edge_weight = _graph_tensor(graph, 'edge_weight')
    if edge_index is None or edge_index.numel() == 0:
    ...
    weights = (
        ...
        else torch.ones(src.size(0), device=device, dtype=dtype)
    )
    for i in range(src.size(0)):
        s = int(src[i].item())
        d = int(dst[i].item())
...
def build_explicit_pair_mask(graph, num_nodes, device, dtype):
    """Binary mask of utterance pairs already linked by explicit edges."""
...
class LatentRelationBranch(nn.Module):
    """
    Predict pairwise latent (implicit) relation scores on topology nodes.
    Teacher side: gold all_label embeddings; student side: stance probability vectors.
    """

    def __init__(self, hidden_dim, num_classes=3, side_dim=32, dropout=0.1):
        super().__init__()
    ...
    def latent_matrix(self, nodes, node_side):
        num_nodes = nodes.size(0)
        latent = nodes.new_zeros(num_nodes, num_nodes)
        if num_nodes <= 1:
            return latent

        for i in range(num_nodes):
            for j in range(i + 1, num_nodes):
                feat = torch.cat([nodes[i], nodes[j], node_side[i], node_side[j]], dim=-1)
                score = torch.sigmoid(self.pair_scorer(feat).squeeze(-1))
                latent[i, j] = score
                latent[j, i] = score
        return latent


class ImplicitTopologyGCN(nn.Module):
    """
    Distill latent relation matrix (train-only teacher), add implicit edges,
    then run GCN on explicit + implicit adjacency.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.relation = LatentRelationBranch(
            hidden_dim,
            num_classes=num_classes,
            side_dim=side_dim,
            dropout=dropout,
        )
        self.implicit_edge_weight = implicit_edge_weight
        self.gc1 = GraphConvolution(hidden_dim, hidden_dim)
        self.gc2 = GraphConvolution(hidden_dim, hidden_dim)
        self.dropout = nn.Dropout(dropout)
        self.norm = nn.LayerNorm(hidden_dim)

    def forward(self, nodes, graph, node_side_student, node_side_teacher=None):
        num_nodes = nodes.size(0)
        device = nodes.device
        dtype = nodes.dtype

        adj_explicit = build_explicit_adjacency(graph, num_nodes, device, dtype)
        pair_mask = build_explicit_pair_mask(graph, num_nodes, device, dtype)

        latent_student = self.relation.latent_matrix(nodes, node_side_student)
        implicit_mask = (1.0 - pair_mask).clamp(min=0.0)
        implicit_mask.fill_diagonal_(0.0)

        adj_implicit = latent_student * implicit_mask
        adj_aug = adj_explicit + self.implicit_edge_weight * adj_implicit
        adj_aug = adj_aug / adj_aug.sum(dim=1, keepdim=True).clamp_min(1e-6)

        hidden = torch.tanh(self.gc1(nodes, adj_aug))
        hidden = self.dropout(hidden)
        hidden = torch.tanh(self.gc2(hidden, adj_aug))
        hidden = self.norm(hidden)

        distill_loss = None
        if node_side_teacher is not None:
            latent_teacher = self.relation.latent_matrix(nodes, node_side_teacher)
            mask = implicit_mask > 0
            if mask.any():
                distill_loss = F.mse_loss(
                    latent_student[mask],
                    latent_teacher.detach()[mask],
                )

        return hidden, distill_loss, latent_student
